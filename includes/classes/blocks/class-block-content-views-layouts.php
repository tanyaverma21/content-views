...
class Block_Content_Views_Layouts {
	...
	public function render_callback( $attributes ) {
		...
		$selected_posts = isset( $attributes['selectedPosts'] ) && ! empty( $attributes['selectedPosts'] ) ? array_column($attributes['selectedPosts'], 'id') : [];
		...
		ob_start();
		?>
		<div class="wp-block-content-views-layouts-view view-block <?php echo esc_attr($post_layout) ?>"><?php
			$query_args = [
				'post_type' => $post_type,
				'orderby' => $orderby,
				'order' => $order,
			...
			if ( $post_query->have_posts() ) {
				while ( $post_query->have_posts() ) {
					...
					$post_taxonomies = [];
					$post_thumbnail = \has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'post-thumbnail') : CV_PLACEHOLDER_THUMB_IMG;
					$count++;
					$index++;
					if ('double-line-scroll' === $post_layout && $count === 1) { ?>
						<div class="slide-post-item">
					<?php } 
					if ($index === 1 && ('horizontal' === $post_layout || 'vertical' === $post_layout)) { ?>
						<div class="large-container"><?php
					} 
					if ($index === 2 && ('horizontal' === $post_layout || 'vertical' === $post_layout)) { ?>
						<div class="small-containers"><?php
					} ?>
					<div class="post-item">
						...
						if ('tiles' === $post_layout || 'grid' === $post_layout) {
							$this->output_grid_tiles_layout($post_taxonomies, $display_taxonomies, $post_layout, $display_date, $display_author, $post_id);
						} else if ('collapsible' === $post_layout) {
							$this->output_collapsible_layout($post_taxonomies, $display_taxonomies, $post_layout, $display_date, $display_author, $post_id);
						} else if ('single-line-scroll' === $post_layout || 'double-line-scroll' === $post_layout) {
							$this->output_scrollable_layout($post_taxonomies, $display_taxonomies, $post_layout, $display_date, $display_author, $post_id);
						} else if ('horizontal' === $post_layout || 'vertical' === $post_layout) {
							$this->output_horizontal_layout($post_taxonomies, $display_taxonomies, $post_layout, $display_date, $display_author, $post_id, $index);
						}
								...
								<div class="terms">
									...
									if (!empty($taxonomies)):
										foreach($taxonomies as $taxonomy):
											$taxos = wp_get_post_terms($post_id, $taxonomy,  array("fields" => "names"));
											if (!empty($taxos)) { ?>
												<span><?php echo esc_html(ucfirst($taxonomy)) ?>: </span><?php echo esc_html(implode(', ', $taxos));
											} ?>
										<?php endforeach;
									endif; ?>
								</div>
					...
					if ('double-line-scroll' === $post_layout && $count === 2) { ?>
						</div><?php
						$count = 0;
					}
					if (($index === 6 || $index === 1) && ('horizontal' === $post_layout || 'vertical' === $post_layout)) { ?>
						</div><?php
					}
				} ?>
	 ...
	 * @since 1.0.0
	 */
	public function output_grid_tiles_layout( $post_taxonomies, $display_taxonomies, $post_layout, $display_date,  $display_author, $post_id ) { 
		$post_thumbnail = \has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'post-thumbnail') : CV_PLACEHOLDER_THUMB_IMG;
	 ...
	 * @since 1.0.0
	 */
	public function output_collapsible_layout( $post_taxonomies, $display_taxonomies, $post_layout, $display_date,  $display_author, $post_id ) { 
		$post_thumbnail = \has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'post-thumbnail') : CV_PLACEHOLDER_THUMB_IMG; ?>
	 ...
	 * @since 1.0.0
	 */
	public function output_scrollable_layout( $post_taxonomies, $display_taxonomies, $post_layout, $display_date,  $display_author, $post_id ) { 
		$post_thumbnail = \has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'post-thumbnail') : CV_PLACEHOLDER_THUMB_IMG; ?>
		<div class="scroll-header"><?php
			if (!empty($post_taxonomies) && $display_taxonomies) { ?>
				<div class="post-entities">
	 ...
	 * @since 1.0.0
	 */
	public function output_horizontal_layout( $post_taxonomies, $display_taxonomies, $post_layout, $display_date,  $display_author, $post_id, $index ) { 
		$post_thumbnail = \has_post_thumbnail($post_id) ? get_the_post_thumbnail_url($post_id, 'post-thumbnail') : CV_PLACEHOLDER_THUMB_IMG;
